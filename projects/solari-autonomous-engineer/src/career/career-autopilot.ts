...
export const PRASHANTH_PROFILE: CandidateProfile = {
...
export const SAMPLE_LIVE_JOB_TARGETS: JobTarget[] = [
  {
    id: "job_fadv_01",
...
export class CareerAutopilot {
  ...
  constructor(
    private solariApiKey: string = process.env.SOLARI_API_KEY || "",
    private profile: CandidateProfile = PRASHANTH_PROFILE,
  ...
  private emit(step: number, phase: AgentStepEvent["phase"], action: string, details?: Record<string, unknown>): void {
    if (this.onStep) {
      this.onStep({
        step,
   ...
   */
  async applyToJob(job: JobTarget, dryRun = true): Promise<ApplicationResult> {
...
Portfolio: ${this.profile.portfolioUrl} | GitHub: ${this.profile.githubUrl}`;
      ...
      this.emit(
        step++,
        "browser_qa",
        `Launching Solari Stealth Cloud Browser (US Residential Egress) to open ${job.portalType.toUpperCase()} portal...`,
      );
      ...
      const tailoredAnswers: Record<string, string> = {
   ...
   */
  async runDailyBatch(targets: JobTarget[] = SAMPLE_LIVE_JOB_TARGETS, dryRun = true): Promise<ApplicationResult[]> {
