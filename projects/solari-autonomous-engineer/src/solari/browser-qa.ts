import { Solari, type BrowserSession, type CreateSessionOptions } from "@solarisdk/browser";
import type { QAAssertion, QAResult } from "../types.js";

export interface BrowserQAOptions {
  stealth?: boolean;
  proxy?: string | { country: string; tier?: string; session?: string };
  recording?: boolean;
  apiKey?: string;
...
export class BrowserQA {
   ...
   */
  async launchQABrowser(options: BrowserQAOptions = {}): Promise<string> {
    ...
    try {
      this.browser = await this.solari.launch({
        stealth,
        recording,
        proxy: options.proxy as CreateSessionOptions["proxy"],
      });
   ...
   */
  async verifyLivePreview(previewUrl: string, assertions: QAAssertion[] = []): Promise<QAResult> {
    const startTime = Date.now();
    const consoleLogs: { type: string; text: string }[] = [];
    const assertionResults: QAResult["assertions"] = [];
   ...
   */
  async extractSessionReplay(sessionId: string): Promise<Array<Record<string, unknown>>> {
    if (this.isSimulated || !this.solari) {
      return [
    ...
    for (let attempt = 1; attempt <= 10; attempt++) {
      ...
      try {
        ...
        const statusVal = isErrObj && "status" in err ? (err as Record<string, unknown>).status : undefined;
